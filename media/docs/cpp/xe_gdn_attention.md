# Xe Chunkwise Gated DeltaNet (GDN) Attention

A direct CuTe-on-SYCL port of the Gated DeltaNet (GDN) **chunkwise
attention forward pass** for Intel Xe GPUs. The kernel runs as five raw
...
| Consumer | Path | Role |
|---|---|---|
| Example | [`examples/14_xe_gdn_attention/`](../../../examples/14_xe_gdn_attention) | Driver + runner; verifies against the shared host reference |
...
Each call to
[`cutlass::gdn::chunk_gated_delta_rule_launch<T, StateT>`](../../../applications/gdn_attention/xe_chunk_gated_delta_rule.hpp)
...
[`cutlass::gdn::get_workspace_sizes()`](../../../applications/gdn_attention/xe_chunk_gated_delta_rule.hpp).
...
header `xe_chunk_gated_delta_rule.hpp` only *declares* the entry point and is
for host-only consumers that just need `GDNArguments` / `get_workspace_sizes`.

```cpp
#include "gdn_attention/xe_chunk_gated_delta_rule_launch.hpp"
...
[`GDNArguments`](../../../applications/gdn_attention/xe_chunk_gated_delta_rule.hpp)
...
[`get_workspace_sizes(args)`](../../../applications/gdn_attention/xe_chunk_gated_delta_rule.hpp)
...
[`xe_chunk_gated_delta_rule.hpp`](../../../applications/gdn_attention/xe_chunk_gated_delta_rule.hpp);
the launch in step 4 needs the [launch
header](../../../applications/gdn_attention/xe_chunk_gated_delta_rule_launch.hpp).
...
- **Only full-chunk sequences are supported.** Per-batch `seq_len` **must be a
  multiple of `kChunkSize`** (currently 64: valid lengths are 64, 128, 256, 512, ...).
  Partial-tail sequences where `seq_len % kChunkSize != 0` (e.g., 200 = 3×64 + 8-row tail)
  are **not validated** — they exhibit numerical differences between the kernel
  output and both the recurrent and chunkwise fp32 reference oracles on some seeds
  (~0.05% of SSM state elements exceed the 5% tolerance band, though typically
  within 10-20%). **only chunk-aligned sequence lengths are
  supported for correctness validation**. The kernel will not fault on
  non-multiples of `kChunkSize` (padding is applied automatically), but numerical
...
```
  ...
  examples/14_xe_gdn_attention ─┐
  benchmarks/gdn ───────────────┼─include─▶ xe_chunk_gated_delta_rule_launch.hpp
  test/unit/gdn_attention ──────┘            (inline launcher: validate + dispatch)
                                                  │ includes
                                                  ▼
                                             xe_chunk_gated_delta_rule_kernels.hpp
                                             (5 device kernels + kernel_launcher)
                                                  │ uses
                                                  ▼
                                             xe_chunk_gated_delta_rule_gemm.hpp
                                                  (CuTe GEMM helpers)

  host reference / shape-only ──include─▶ xe_chunk_gated_delta_rule.hpp
...
| File | Purpose |
|---|---|
| [xe_chunk_gated_delta_rule.hpp](../../../applications/gdn_attention/xe_chunk_gated_delta_rule.hpp) | Lightweight public API: `GDNArguments`, `get_workspace_sizes`, `chunk_gated_delta_rule_launch` declaration |
| [xe_chunk_gated_delta_rule_launch.hpp](../../../applications/gdn_attention/xe_chunk_gated_delta_rule_launch.hpp) | Header-only launcher: argument validation + `chunk_gated_delta_rule_launch<T, StateT>` definition (inline template, instantiated at each call site) |
| [xe_chunk_gated_delta_rule_kernels.hpp](../../../applications/gdn_attention/xe_chunk_gated_delta_rule_kernels.hpp) | Five device kernels + `detail::kernel_launcher` (upstream-aligned signature) |
| [xe_chunk_gated_delta_rule_gemm.hpp](../../../applications/gdn_attention/xe_chunk_gated_delta_rule_gemm.hpp) | CuTe GEMM helpers (`gemm_TTS`, `gemm_STS`, `gemm_TSS`, `gemm_TTS_k_multi`) |
| [xe_gdn_attention_stage_references.hpp](../../../tools/util/include/cutlass/util/reference/host/xe_gdn_attention_stage_references.hpp) | Per-stage host reference implementations (in `cutlass/util/reference/host/`) shared by the example and unit test |
| [xe_gdn_attention_compare.hpp](../../../tools/util/include/cutlass/util/reference/host/xe_gdn_attention_compare.hpp) | Host verification comparator (`compare_with_stats`/`print_compare_stats`, in `cutlass/util/reference/host/`) used by the example runner |
| [examples/14_xe_gdn_attention/](../../../examples/14_xe_gdn_attention) | Example driver, runner, and perf helpers (verifies via the shared host reference above) |
