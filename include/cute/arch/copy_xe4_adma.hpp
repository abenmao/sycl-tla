...
struct XE4_ASYNC_TENSOR_LOAD
{
  template <typename DataType, uint64_t Dim>
  ...
  copy(
    ...
    mat_desc_.StartAddress = (uint32_t)((uint64_t)slm_space_cast(slm_ptr) >> 9);

    detail::AsyncTensorGlobal2SLM<DataType, detail::CacheCtrl::L2c_L3uc, detail::FillMethod::Zero>::
      copy(mat_desc_, gmem_ptr, abar_ptr, (TensorDescriptor *)tdesc_ptr, coord);
  }
};
...
struct XE4_ASYNC_TENSOR_STORE
{
  ...
  copy(
    ...
    detail::AsyncTensorSLM2Global<sizeof(DataType) * 8, detail::CacheCtrl::L2wb_L3uc>::
      copy(gmem_ptr, mat_desc_, abar_ptr, (TensorDescriptor *)tdesc_ptr, coord);
  }
};
...
struct XE4_ASYNC_TENSOR_LOAD_MULTICAST
{
  template<typename DataType, uint64_t Dim>
  CUTE_HOST_DEVICE static void
  copy(
    ...
    MatrixDescriptor mat_desc_ (mat_desc);
    mat_desc_.StartAddress = static_cast<uint16_t>(reinterpret_cast<uint64_t>(slm_ptr)>>9);

    detail::AsyncTensorGlobal2SLM<DataType, detail::CacheCtrl::L2c_L3uc, detail::FillMethod::Zero>::
      copy(mat_desc_, gmem_ptr, abar_ptr, (TensorDescriptor *)tdesc_ptr, coord, multicast_mask);
  }
};
