...
inline void tensordesc_fill_dim_stride(tdesc_ptr_t tdesc_ptr, const strides_t &dim_strides) {
  static_assert(dim_num >= 1 && dim_num <= 5, "Unsupported dimention size for tensor descriptor dim_stride");
  static_assert(sizeof(strides_t {}[0]) == sizeof(uint64_t), "Invalid stride type");
  if constexpr (dim_num > 1) {
    INLINE_PISA("tensordesc.fill.dim_stride.64b [%0], 1, %1;" ::"r"(tdesc_ptr), "r"(dim_strides[1]));
  }
  if constexpr (dim_num > 2) {
    INLINE_PISA("tensordesc.fill.dim_stride.64b [%0], 2, %1;" ::"r"(tdesc_ptr), "r"(dim_strides[2]));
  }
  if constexpr (dim_num > 3) {
    INLINE_PISA("tensordesc.fill.dim_stride.64b [%0], 3, %1;" ::"r"(tdesc_ptr), "r"(dim_strides[3]));
  }
  if constexpr (dim_num > 4) {
    INLINE_PISA("tensordesc.fill.dim_stride.64b [%0], 4, %1;" ::"r"(tdesc_ptr), "r"(dim_strides[4]));
  }
}
...
inline void async_tensor_load(tdesc_t tdesc, mat_desc_t mat_desc, dtype *gmem_ptr,
  ...
  if constexpr (dim == 1) {
    if constexpr (sizeof_bits<dtype>() == 4) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.1d.4b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 6) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.1d.6b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 8 || sizeof(dtype) == 1) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.1d.8b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 16) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.1d.16b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 32) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.1d.32b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 64) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.1d.64b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else {
    ...
    if constexpr (sizeof(dtype) == 1) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.5d.8b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof(dtype) == 2) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.5d.16b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof(dtype) == 4) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.5d.32b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof(dtype) == 8) {
      INLINE_PISA(
          "async_tensor_copy.shared_workgroup.global.5d.64b.uint.zero.L2c.L3uc.abarrier %0, [%1], [%2], [%3], "
          "%4;" ::"r"(mat_desc),
          "r"(gmem_ptr), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else {
      static_assert(false, "Unsupported data size");
...
inline void async_tensor_store(tdesc_t tdesc, mat_desc_t mat_desc, dtype *gmem_ptr,
  ...
  if constexpr (dim == 1) {
    if constexpr (sizeof_bits<dtype>() == 4) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.1d.4b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 6) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.1d.6b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 8) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.1d.8b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 16) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.1d.16b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 32) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.1d.32b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 64) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.1d.64b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else {
      static_assert(false, "Unsupported data size");
    ...
    if constexpr (sizeof_bits<dtype>() == 4) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.5d.4b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 8) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.5d.8b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 16) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.5d.16b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 32) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.5d.32b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else if constexpr (sizeof_bits<dtype>() == 64) {
      INLINE_PISA("async_tensor_copy.global.shared_workgroup.5d.64b.L2wb.L3uc.abarrier [%0], %1, [%2], [%3], %4;" ::"r"(
                      gmem_ptr),
                  "r"(mat_desc), "r"(abar), "r"(tdesc), "r"(as_vector_t(coord)));
    } else {
      static_assert(false, "Unsupported data size");
