...
template <class TensorDesc, class AuxParams, class GmemPtr>
struct Xe4DmaCache {
...
template <class TmaInternalType, class TensorDesc, class GEngine, class GLayout, class SLayout>
CUTE_HOST_DEVICE auto
initialize_tensor_desc(TensorDesc tensor_desc, Tensor<GEngine, GLayout> const& gtensor, SLayout const& slayout, uint32_t coop_size)
{
  using ValueType = typename GEngine::value_type;

  constexpr bool isMajorK = cutlass::gemm::detail::is_k_major<decltype(stride(gtensor))>();
  constexpr int ldm = static_cast<int>(isMajorK);
  ...
  if (block_width == 1 || block_height_ == 1) {
    block_width = size<0, ldm>(slayout_recasted);
    block_height_ = size<0, non_ldm>(slayout_recasted);
  }
...
initialize_tensor_desc_conv2d(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout, uint32_t coop_size = 1)
{
  ...
  sycl::vec<uint32_t, 4> gmem_shape;
  sycl::vec<uint64_t, 3> gmem_stride;
  sycl::vec<uint32_t, 4> roi_shape;

  if constexpr (cutlass::gemm::detail::is_k_major<decltype(stride(gtensor))>()) {
    gmem_shape = {(uint32_t)shape<1, 0>(gtensor),
      (uint32_t)shape<1, 2>(gtensor), (uint32_t)shape<1, 1>(gtensor), (uint32_t)shape<0>(gtensor)};
    gmem_stride = {stride<1, 2>(gtensor) * sizeof(T), stride<1, 1>(gtensor) * sizeof(T),
      stride<0>(gtensor) * sizeof(T)};
    roi_shape = {shape<1>(slayout), 1, 1, shape<0>(slayout) / coop_size};
  } else {
    gmem_shape = {(uint32_t)shape<0>(gtensor),
      (uint32_t)shape<1, 2>(gtensor), (uint32_t)shape<1, 1>(gtensor), (uint32_t)shape<1, 0>(gtensor)};
    gmem_stride = {stride<1, 2>(gtensor) * sizeof(T), stride<1, 1>(gtensor) * sizeof(T),
      stride<1, 0>(gtensor) * sizeof(T)};
    roi_shape =  {shape<0>(slayout) / coop_size, 1, 1, shape<1>(slayout)};
  }
  sycl::vec<uint32_t, 4> elem_stride {1, 1, 1, 1};

  tensordesc_fill_dim_size<4>(tensor_desc, gmem_shape);
  tensordesc_fill_dim_stride<4>(tensor_desc, gmem_stride);
  tensordesc_fill_element_stride<4>(tensor_desc, elem_stride);
  tensordesc_fill_roitensor_dim_size<4>(tensor_desc, roi_shape);
}

template <class CopyOp, class TmaInternalType, class TensorDesc, class GEngine, class GLayout, class SLayout, class VLayout>
CUTE_HOST_DEVICE auto
make_copy_atom(TensorDesc tensor_desc, Tensor<GEngine, GLayout> const& gtensor, SLayout const& slayout, uint32_t coop_size, VLayout const& cta_v_map)
{
  ...
  initialize_tensor_desc<TmaInternalType>(tensor_desc, gtensor, slayout, coop_size);
  ...
  using DmaCache = Xe4DmaCache<decltype(tensor_desc), decltype(aux_params), decltype(gmem_ptr)>;
...
template <class CopyOp, class TensorDesc, class GEngine, class GLayout, class SLayout, class VLayout>
CUTE_HOST_DEVICE auto
make_copy_atom_conv2d(TensorDesc tensor_desc, Tensor<GEngine, GLayout> const& gtensor, SLayout const& slayout,
uint32_t coop_size, VLayout const& cta_v_map)
{
  ...
  initialize_tensor_desc_conv2d(tensor_desc, gtensor, slayout);
...
template <class CopyOp, class TmaInternalType, class TensorDesc, class GTensor, class SLayout, class TLayout, class VLayout>
CUTE_HOST_RTC auto
make_xe4_copy_tiled(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout,
                    TLayout const& cta_t_map, VLayout const& cta_v_map)
{
  auto cta_tiler = product_each(shape(cta_v_map));
  auto atom = make_copy_atom<CopyOp, TmaInternalType>(tensor_desc, gtensor, slayout, size(cta_t_map), cta_v_map);
...
template <class CopyOp, class TmaInternalType, class TensorDesc, class GTensor, class SLayout, class TLayout, class VLayout>
CUTE_HOST_RTC auto
make_xe4_copy_tiled_conv2d(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout,
                    TLayout const& cta_t_map, VLayout const& cta_v_map)
{
  auto cta_tiler = product_each(shape(cta_v_map));
  auto atom = make_copy_atom_conv2d<CopyOp>(tensor_desc, gtensor, slayout, cosize(cta_t_map), cta_v_map);
...
template <class CopyOp, class TmaInternalType = void, class TensorDesc, class GTensor, class SLayout, class Tiler, class Cluster_Size>
CUTE_HOST_RTC auto
make_xe4_copy(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout, Tiler const& cta_tiler, Cluster_Size const& cluster_size)
{
  ...
  return make_xe4_copy_tiled<CopyOp, TmaType>(tensor_desc, gtensor, slayout, cta_t_tile, cta_v_tile);
}

template <class CopyOp, class TmaInternalType = void, class TensorDesc, class GTensor, class SLayout, class Tiler>
CUTE_HOST_RTC auto
make_xe4_copy(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout, Tiler const& cta_tiler)
{
  return make_xe4_copy<CopyOp, TmaInternalType>(tensor_desc, gtensor, slayout, cta_tiler, _1{});
}

template <class CopyOp, class TmaInternalType = void, class TensorDesc, class GTensor, class SLayout, class Tiler, class Cluster_Size>
CUTE_HOST_RTC auto
make_xe4_copy_conv2d(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout, Tiler const& cta_tiler, Cluster_Size const& cluster_size)
{
  auto cta_v_tile = make_identity_layout(shape(gtensor)).compose(cta_tiler);
  auto cta_t_tile = make_layout(cluster_size);
  // Prefer TmaInternalType if specified. Fallback to GEngine::value_type
  using TmaType = conditional_t<is_same<void, TmaInternalType>::value, typename GTensor::value_type, TmaInternalType>;
  return make_xe4_copy_tiled_conv2d<CopyOp, TmaType>(tensor_desc, gtensor, slayout, cta_t_tile, cta_v_tile);
}

template <class CopyOp, class TensorDesc, class TmaInternalType = void, class GTensor, class SLayout, class Tiler>
CUTE_HOST_RTC auto
make_xe4_copy_conv2d(TensorDesc tensor_desc, GTensor const& gtensor, SLayout const& slayout, Tiler const& cta_tiler)
{
  return make_xe4_copy_conv2d<CopyOp, TmaInternalType>(tensor_desc, gtensor, slayout, cta_tiler, _1{});
}
...
CUTE_HOST
auto
make_tma_atom_A_xe4(CopyOp                  const& copy_op,
                    ...
                    ClusterShapeVMNK        const& cluster_shape)  // (CTA_V, CTA_M, CTA_N, CTA_K)
{
  ...
  return detail::make_copy_atom<CopyOp, TmaType>(tensor_desc, gtensor, slayout, num_multicast, cta_v_tile);
}
...
CUTE_HOST
auto
make_tma_atom_B_xe4(CopyOp                  const& copy_op,
                    TensorDesc                     tensor_desc,
