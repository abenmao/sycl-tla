...
/*!
  \file xe_gdn_attention_stage_references.hpp
  ...
  Shared by the two host-side verifiers of the GDN kernel:
    - examples/14_xe_gdn_attention/xe_gdn_attention_runner.hpp
  ...
  Why a chunkwise (not recurrent) reference
  -----------------------------------------
  A per-token recurrence reference is algebraically equivalent to the chunkwise
  form, but in bf16 the two accumulate floating-point error differently (the
  recurrent form drifts to rel_rms ~ 0.21 on core_attn_out / ~ 0.25 on
  ssm_state against the chunkwise kernel). Mirroring the kernel's own chunkwise
  decomposition keeps the reference at the 5e-2 tolerance the kernel meets.
...
// (xe_chunk_gated_delta_rule_kernels.hpp) so it carries no dependency back into
// applications/ and no device code leaks into host translation units.
#include "gdn_attention/xe_chunk_gated_delta_rule.hpp"  // for cutlass::gdn::kChunkSize
 ...
 * (kernel_launcher in xe_chunk_gated_delta_rule_kernels.hpp). Inputs match
